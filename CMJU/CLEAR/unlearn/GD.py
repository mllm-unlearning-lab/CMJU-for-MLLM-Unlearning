...
def main(args):
    ...
    accelerator.print(f"[Model] Loading base model from: {args.base_model_dir}")
    ...
    accelerator.print(f"[Model] Loading pre-unlearning LoRA from: {args.lora_dir}")
    ...
    accelerator.print(
        f"[Data] forget raw={len(forget_raw)} | retain raw={len(retain_raw)}"
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        for mini_step, caption_f in enumerate(bar):
            ...
            out_f_m = _forward(caption_f, model, "multimodal")
            loss_m_f = out_f_m.loss
            accelerator.backward(-loss_m_f / args.grad_accum_steps)
            del out_f_m

            loss_m_r_log = 0.0
            for _ in range(args.retain_steps_per_forget):
                r_cap = next(retain_caption_iter)
                out_r_m = _forward(r_cap, model, "multimodal")
                loss_m_r = out_r_m.loss
                accelerator.backward(
                    (loss_m_r / args.retain_steps_per_forget) / args.grad_accum_steps
                )
                loss_m_r_log += loss_m_r.item()
                del out_r_m
            loss_m_r_log /= max(1, args.retain_steps_per_forget)

            out_f_u = _forward(text_f, model, "unimodal")
            loss_u_f = out_f_u.loss
            accelerator.backward(-loss_u_f / args.grad_accum_steps)
            del out_f_u

            loss_u_r_log = 0.0
            for _ in range(args.retain_steps_per_forget):
                r_txt = next(retain_text_iter)
                out_r_u = _forward(r_txt, model, "unimodal")
                loss_u_r = out_r_u.loss
                accelerator.backward(
                    (loss_u_r / args.retain_steps_per_forget) / args.grad_accum_steps
                )
                loss_u_r_log += loss_u_r.item()
                del out_r_u
            loss_u_r_log /= max(1, args.retain_steps_per_forget)

            step_log = loss_m_f.item() + loss_u_f.item() + loss_m_r_log + loss_u_r_log
            total_log += step_log
            ...
            bar.set_postfix(
                ...
                opt=n_opt,
                f_m=f"{loss_m_f.item():.3f}",
                f_u=f"{loss_u_f.item():.3f}",
                r_m=f"{loss_m_r_log:.3f}",
                r_u=f"{loss_u_r_log:.3f}",
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Pre-unlearning LoRA adapter directory, e.g. .../final_adapter",
    )
    p.add_argument(
        "--save_dir",
        type=str,
